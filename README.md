...
The database schema consists of four relational tables: `CUSTOMERS`, `PRODUCTS`, `ORDERS`, and `ORDER_ITEMS`. 

<img width="333" height="414" alt="image" src="https://github.com/user-attachments/assets/570306ff-a4c5-4055-8546-5f377c57f0fc" />

**Figure 1:** *This shows table creation scripts* 

<img width="378" height="167" alt="image" src="https://github.com/user-attachments/assets/2ac46fa5-0222-4d03-8d4d-cc0e2824fb23" />

**Figure 2:** *This figure shows all created tables* 

### **Sample Data Insertion** 

The database was populated with sample data using Oracle's `INSERT ALL` statement to load records across all four tables efficiently. 

<img width="605" height="154" alt="image" src="https://github.com/user-attachments/assets/07a32fb8-7b94-469a-9204-9a27108b3e70" />

**Figure 3:** *This figure shows customer data entry* 
...
**Figure 4:** *This figure shows product data entry* 
...
**Figure 5:** *This figure shows order_items data entry* 

---

## **3. Transition from Command Prompt to Oracle APEX** 

### **Reason for Shifting to APEX** 

Command Prompt (`sqlplus`) is ideal for running creation and insertion scripts, but its text interface often line-wraps query results and makes tables hard to read. 
...
#### **Query 1: Order Details with Customer Names (INNER JOIN)** 
- **Explanation:** Combines `ORDERS` and `CUSTOMERS` to display each order alongside the customer's name and city. 
...
#### **Query 2: Order Items with Product Details (JOIN)** 
- **Explanation:** Joins `ORDER_ITEMS` with `PRODUCTS` to display product names, categories, unit prices, and quantities for each ordered item. 
...
#### **Query 3: Customers and Their Orders (LEFT JOIN)** 
- **Explanation:** Displays all registered customers, including those who have not yet placed an order (shown with `NULL` values in order columns). 
...
#### **Query 4: High-Value Customers (Spending Above Average)** 
...
#### **Query 5: Ranking Customers by Spend (`DENSE_RANK`)** 
- **Explanation:** Ranks customers from highest to lowest spender using `DENSE_RANK()`, giving tied amounts the same rank without leaving gaps. 
...
#### **Query 6: Sequential Order History (`ROW_NUMBER`)** 
- **Explanation:** Numbers each customer's orders sequentially (`1, 2, 3...`) based on transaction date. 
...
#### **Query 7: Running Revenue Over Time (`SUM OVER`)** 
- **Explanation:** Groups sales by order date and calculates the cumulative running total revenue across days. 
...
#### **Query 8: Order Recency & Retention Analysis (`LAG`)** 
- **Explanation:** Uses `LAG()` to look up a customer's previous order date and calculates the number of days elapsed between consecutive orders. 
